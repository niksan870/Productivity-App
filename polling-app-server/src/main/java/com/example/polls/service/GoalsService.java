...
//                updateGoal.setUpdatedAt();
    ...
    public Page<GoalResponse> getPage(int pageNo, int pageSize, String filterParams) {
        ...
        Page<Goal> goals;
        if (!all) {
            goals = goalsRepository.findWhereUserIdWithFilter(userPrincipal.getCurrentUserPrincipal().getId(), q, pageable);
        } else {
            goals = goalsRepository.findWithFilter(userPrincipal.getCurrentUserPrincipal().getId(), q, pageable);
        }
        Map<Long, User> creatorMap = getGoalCreatorMap(goals.getContent());

        List<Goal> test = goals.getContent();


        List<GoalResponse> goalResponses = goals.map(goal -> {
            try {
        ...
        List<EntityModel<Goal>> employees = test.stream()
                .map(goalResourceAssembler::toModel)
                .collect(Collectors.toList());

        final Page<GoalResponse> page = new PageImpl<>(goalResponses);
    ...
    public void sendRequest(UUID id) {
        ...
        List<User> updateList = goal.getAttendees();
    ...
    Map<Long, User> getGoalCreatorMap(List<Goal> goals) {
